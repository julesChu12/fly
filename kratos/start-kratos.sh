...
DEFAULT_GRPC_PORT="9092"
DEFAULT_CONFIG="configs/kratos.yaml"
DEFAULT_ENV="development"

# 显示帮助
show_help() {
    echo -e "${BLUE}Kratos 订单管理服务启动脚本${NC}"
    echo ""
    echo "使用方法:"
    echo "  $0 [选项]"
    echo ""
    ...
    echo "  -e, --env <env>         运���环境 (默认: $DEFAULT_ENV)"
...
if [ "$STOP_SERVICE" = true ]; then
    echo -e "${BLUE}正在停止 Kratos 服务...${NC}"

    # 查找并停止进程
    KRATOS_PIDS=$(ps aux | grep "[k]ratos.*serve" | grep -v grep | awk '{print $2}')

    if [ -n "$KRATOS_PIDS" ]; then
        echo -e "${YELLOW}找到以下进程:${NC}"
        echo "$KRATOS_PIDS"

        echo -n "正在停止..."
        echo "$KRATOS_PIDS" | xargs kill -TERM 2>/dev/null || true
        sleep 2

        # 如果进程仍在运行，强制杀死
        REMAINING=$(ps aux | grep "[k]ratos.*serve" | grep -v grep | awk '{print $2}' | wc -l)
        if [ "$REMAINING" -gt 0 ]; then
            echo "强制停止剩余进程..."
            ps aux | grep "[k]ratos.*serve" | grep -v grep | awk '{print $2}' | xargs kill -9 2>/dev/null || true
        fi

        echo -e "${GREEN}✅ Kratos 服务已停止${NC}"
    else
        echo -e "${YELLOW}⚠️ 没有找到运行中的 Kratos 服务${NC}"
    fi
...
check_port() {
    ...
    if netstat -an 2>/dev/null | grep ":$port " | grep -q "LISTEN"; then
        ...
        echo -e "${YELLOW}请使用 'pkill -f \"kratos.*serve\"' 停止相关进程${NC}"
        exit 1
    fi
}
...
if [ "$ENVIRONMENT" != "$DEFAULT_ENV" ]; then
    CMD="KRATOS_ENV=$ENVIRONMENT $CMD"
fi

# 构建日志选项
LOG_FILE="logs/kratos_$(date +%Y%m%d_%H%M%S).log"
...
# 显示启动信息
echo -e "${BLUE}================================${NC}"
echo -e "${BLUE}  Kratos 订单管理服务启动${NC}"
echo -e "${BLUE}================================${NC}"
echo ""
echo -e "${BLUE}配置信息:${NC}"
...
# 获取进程ID
KRATOS_PID=$!
echo -e "${GREEN}✅ Kratos 服务已启动 (PID: $KRATOS_PID)${NC}"
...
# 保存PID到文件
echo $KRATOS_PID > kratos.pid
echo -e "${BLUE}PID 已保存到: kratos.pid${NC}"