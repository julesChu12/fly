...
DEFAULT_CONFIG="configs/custos.yaml"
DEFAULT_ENV="development"

# 显示帮助
show_help() {
    echo -e "${BLUE}Custos 认证服务启动脚本${NC}"
    echo ""
    echo "使用方法:"
    echo "  $0 [选项]"
    echo ""
...
if [ "$STOP_SERVICE" = true ]; then
    echo -e "${BLUE}正在停止 Custos 服务...${NC}"

    # 查找并停止进程
    CUSTOS_PIDS=$(ps aux | grep "[u]serd main.go serve" | grep -v grep | awk '{print $2}')

    if [ -n "$CUSTOS_PIDS" ]; then
        echo -e "${YELLOW}找到以下进程:${NC}"
        echo "$CUSTOS_PIDS"

        echo -n "正在停止..."
        echo "$CUSTOS_PIDS" | xargs kill -TERM 2>/dev/null || true
        sleep 2

        # 如果进程仍在运行，强制杀死
        REMAINING=$(ps aux | grep "[u]serd main.go serve" | grep -v grep | awk '{print $2}' | wc -l)
        if [ "$REMAINING" -gt 0 ]; then
            echo "强制停止剩余进程..."
            ps aux | grep "[u]serd main.go serve" | grep -v grep | awk '{print $2}' | xargs kill -9 2>/dev/null || true
        fi

        echo -e "${GREEN}✅ Custos 服务已停止${NC}"
    else
        echo -e "${YELLOW}⚠️ 没有找到运行中的 Custos 服务${NC}"
    fi

    # 清理端口占用
    echo -n "检查端口占用..."
    for port in 9001 9002 9003 8081; do
...
check_port() {
    ...
    if netstat -an 2>/dev/null | grep ":$port " | grep -q "LISTEN"; then
        ...
        echo -e "${YELLOW}请使用 'pkill -f \"userd main.go\"' 停止相关进程${NC}"
        exit 1
    fi
}
...
# 构建启动命令
CMD="go run ./cmd/userd/main.go serve"
...
if [ "$ENVIRONMENT" != "$DEFAULT_ENV" ]; then
    CMD="CUSTOS_ENV=$ENVIRONMENT $CMD"
fi

# 构建日志选项
LOG_FILE="logs/custos_$(date +%Y%m%d_%H%M%S).log"
...
# 获取进程ID
CUSTOS_PID=$!
echo -e "${GREEN}✅ Custos 服务已启动 (PID: $CUSTOS_PID)${NC}"
...
echo -e "  健康检查: ${GREEN}http://localhost:$HTTP_PORT/api/v1/health${NC}"
echo ""
echo -e "${BLUE}常用命令:${NC}"
...
# 保存PID到文件
echo $CUSTOS_PID > custos.pid
echo -e "${BLUE}PID 已保存到: custos.pid${NC}"