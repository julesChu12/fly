...
DEFAULT_GRPC_PORT="9084"
DEFAULT_CONFIG="configs/staff.yaml"
DEFAULT_ENV="development"

# 显示帮助
show_help() {
    echo -e "${BLUE}Staff 员工管理服务启动脚本${NC}"
    echo ""
    echo "使用方法:"
    echo "  $0 [选项]"
    echo ""
...
if [ "$STOP_SERVICE" = true ]; then
    echo -e "${BLUE}正在停止 Staff 服务...${NC}"

    # 查找并停止进程
    STAFF_PIDS=$(ps aux | grep "[s]taff.*serve" | grep -v grep | awk '{print $2}')

    if [ -n "$STAFF_PIDS" ]; then
        echo -e "${YELLOW}找到以下进程:${NC}"
        echo "$STAFF_PIDS"

        echo -n "正在停止..."
        echo "$STAFF_PIDS" | xargs kill -TERM 2>/dev/null || true
        sleep 2

        # 如果进程仍在运行，强制杀死
        REMAINING=$(ps aux | grep "[s]taff.*serve" | grep -v grep | awk '{print $2}' | wc -l)
        if [ "$REMAINING" -gt 0 ]; then
            echo "强制停止剩余进程..."
            ps aux | grep "[s]taff.*serve" | grep -v grep | awk '{print $2}' | xargs kill -9 2>/dev/null || true
        fi

        echo -e "${GREEN}✅ Staff 服务已停止${NC}"
    else
        echo -e "${YELLOW}⚠️ 没有找到运行中的 Staff 服务${NC}"
    fi

    # 清理端口占用
    echo -n "检查端口占用..."
    for port in 8084 9084; do
...
check_port() {
    ...
    if netstat -an 2>/dev/null | grep ":$port " | grep -q "LISTEN"; then
        ...
        echo -e "${YELLOW}请使用 'pkill -f \"staff.*serve\"' 停止相关进程${NC}"
        exit 1
    fi
}
...
# 构建启动命令
CMD="go run ./cmd/staff/main.go serve"
...
if [ "$ENVIRONMENT" != "$DEFAULT_ENV" ]; then
    CMD="STAFF_ENV=$ENVIRONMENT $CMD"
fi

# 构建日志选项
LOG_FILE="logs/staff_$(date +%Y%m%d_%H%M%S).log"
...
# 获取进程ID
STAFF_PID=$!
echo -e "${GREEN}✅ Staff 服务已启动 (PID: $STAFF_PID)${NC}"
...
# 保存PID到文件
echo $STAFF_PID > staff.pid
echo -e "${BLUE}PID 已保存到: staff.pid${NC}"