...
// GetByID 根据ID获取预约
func (r *appointmentRepository) GetByID(id string) (*entity.Appointment, error) {
...
func (r *appointmentRepository) Delete(id string) error {
	if err := r.db.Unscoped().Where("id = ?", id).Delete(&entity.Appointment{}).Error; err != nil {
...
func (r *appointmentRepository) SoftDelete(id string) error {
	if err := r.db.Where("id = ?", id).Delete(&entity.Appointment{}).Error; err != nil {
...
// List 获取预约列表
func (r *appointmentRepository) List(filter *dto.AppointmentFilter) ([]*entity.Appointment, error) {
...
// Count 统计预约数量
func (r *appointmentRepository) Count(filter *dto.AppointmentFilter) (int64, error) {
	var count int64
	query := r.buildQuery(filter)

	if err := query.Model(&entity.Appointment{}).Count(&count).Error; err != nil {
...
func (r *appointmentRepository) GetByCustomerID(customerID string, filter *dto.AppointmentFilter) ([]*entity.Appointment, error) {
	var appointments []*entity.Appointment
	query := r.buildQuery(filter)
...
func (r *appointmentRepository) GetByDateRange(startDate, endDate time.Time, filter *dto.AppointmentFilter) ([]*entity.Appointment, error) {
	var appointments []*entity.Appointment
	query := r.buildQuery(filter)
...
func (r *appointmentRepository) GetByStatus(status entity.AppointmentStatus, filter *dto.AppointmentFilter) ([]*entity.Appointment, error) {
	var appointments []*entity.Appointment
	query := r.buildQuery(filter)
...
// UpdateStatus 更新预约状态
func (r *appointmentRepository) UpdateStatus(id string, status entity.AppointmentStatus) error {
	if err := r.db.Model(&entity.Appointment{}).
		Where("id = ?", id).
...
func (r *appointmentRepository) GetPendingReminders(beforeTime time.Time) ([]*entity.Appointment, error) {
	var appointments []*entity.Appointment
	if err := r.db.Where("reminder = ? AND reminder_time <= ? AND start_time > ? AND status IN ? AND deleted_at IS NULL",
		true, beforeTime, time.Now(), []entity.AppointmentStatus{
...
// buildQuery 构建查询条件
func (r *appointmentRepository) buildQuery(filter *dto.AppointmentFilter) *gorm.DB {
	query := r.db.Model(&entity.Appointment{}).Where("deleted_at IS NULL")
